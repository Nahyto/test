	...
	if len(sys.argv) == 1:
		...
		if varAll == 2:
			if len(test_file) != 0:
				...
				email = raw_input("Write the name of your email (Example: \"somebody@something.com\") : ")
				domain = subprocess.check_output("./get_domain.sh %s" %email,shell=True)
				for test in os.listdir("."):
					if test != "automate.py":
						subprocess.call("python %s %s %s" %(test,email,domain),shell=True)
			...
			if len(test_file) == 0:
				...
				for test in test_file:
					subprocess.call("python %s %s %s %s" %(test,email,verbose,domain),shell=True)
